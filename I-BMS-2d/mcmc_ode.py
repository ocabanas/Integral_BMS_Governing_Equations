...
class Tree:
    ...
    def replace_root(self, rr=None, update_gof=True, verbose=False):
        ...
        if update_gof == True:
            ...
            if type(self.fy) != type(None):
                self.fy.sse = self.fy.get_sse(verbose=verbose)
                self.fy.bic = copy(self.bic)
                self.fy.E = copy(self.E)
        return self.root
    ...
    def _add_et(self, node, et_order=None, et=None, update_gof=True, verbose=False):
        ...
        if update_gof == True:
            self.sse = self.get_sse(verbose=verbose)
            self.bic = self.get_bic(verbose=verbose)
            self.E = self.get_energy(verbose=verbose)[0]
            if type(self.fy) != type(None):
                self.fy.sse = self.fy.get_sse(verbose=verbose)
                self.fy.bic = copy(self.bic)
                self.fy.E = copy(self.E)
                ...
                for ds in self.x:
                    ...
                    try:
                        ...
                        if len(parameters_y) != 0:
                            try:
                                this_xhat, this_dxhat = self.fy.dx[ds]
                                xhatmat = [this_xhat[v.name] for v in variables_y] #Coupling smoothed vars
                                dxhatmat = this_dxhat['x'].to_numpy() #This is the coupling smoothed derivative
    ...
    def get_energy(self, bic=False, reset=False, verbose=False):
        ...
        for op, nop in list(self.nops.items()):
            ...
            try:
                ...
                pass
        if type(self.fy) != type(None):
            for op, nop in list(self.fy.nops.items()):
                try:
                    EP += self.fy.prior_par["Nopi_%s" % op] * nop
                except KeyError:
                    pass
                try:
                    EP += self.fy.prior_par["Nopi2_%s" % op] * nop**2
                except KeyError:
                    pass
        ...
        if reset:
            ...
            if type(self.fy) != type(None):
                self.fy.E, self.fy.EB, self.fy.EP = deepcopy(EB+EP), deepcopy(EB), deepcopy(EP)
