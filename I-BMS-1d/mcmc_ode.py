        ...
        if fit:
            if (
                ...
                for ds in self.x:
                    ...
                    this_dx = self.dx[ds][0]
                    
                    # print('this_y,x',this_y)
                    xhatmat = [this_dx[v.name].to_numpy() for v in variables]
                    dxmat = self.dx[ds][1].to_numpy()
                    try:
                        def feval(x, *params):
    ...
    def get_energy(self, bic=False, reset=False, verbose=False):
        ...
        for op, nop in list(self.nops.items()):
            ...
            try:
                ...
                pass
        # Reset the value, if necessaryself.y
        if reset:
            self.EB = EB
            self.EP = EP
            self.E = EB + EP
