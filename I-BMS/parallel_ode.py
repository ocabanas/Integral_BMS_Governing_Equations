...
class Parallel:
    ...
    def __init__(
        ...
        self.t1 = self.trees[self.Ts[0]]
        for BT in [T for T in self.Ts if T != self.Ts[0]]:
            treetmp = Tree(
                ops=ops,
                variables=deepcopy(variables),
                parameters=deepcopy(parameters),
            ...
            self.trees[BT].fit_x0 = self.t1.fit_x0
            self.trees[BT].fit_sse = self.t1.fit_sse
        self.couplings = None
        return
    ...
    def mcmc_step(self, verbose=False, p_rr=0.05, p_long=0.45):
        ...
        for T, tree in list(self.trees.items()):
            # MCMC step
            # print(T,'before step',str(tree),str(tree.fy),tree.E)
            tree.mcmc_step(verbose=verbose, p_rr=p_rr, p_long=p_long)
            # print(T,'after step',str(tree),str(tree.fy),tree.E)
        self.t1 = self.trees[self.Ts[0]]
        # Done
        return
    ...
    def tree_swap(self):
        ...
        BT1, BT2 = t1.BT, t2.BT
        EB1, EB2, EP1, EP2 = t1.EB.get(), t2.EB.get(), t1.EP.get(), t2.EP.get()
        # The energy change
        DeltaE = np.float64(EB1) * (1.0 / BT2 - 1.0 / BT1) + np.float64(EB2) * (
            1.0 / BT1 - 1.0 / BT2
        )
        # DeltaE = np.float64(EB2_-EB1)*(1./BT1) + \
        #         np.float64(EB1_-EB2)*(1./BT1)
        if DeltaE > 0:
            paccept = exp(-DeltaE)
        else:
            paccept = 1.0
        ...
        if random() < paccept:
            ...
            if self.couplings != None:
                #Change the temperatures of the couplings:
                for coupling in self.couplings:
                    if coupling is self:
                        coupling.trees[self.Ts[nT1]], coupling.trees[self.Ts[nT2]] = coupling.trees[self.Ts[nT2]], coupling.trees[self.Ts[nT1]]
                        coupling.trees[self.Ts[nT1]].BT = BT1
                        coupling.trees[self.Ts[nT2]].BT = BT2
    ...
    def set_couplings(self,couplings):
        ...
        # Loop temperatures:
        for Ts in self.Ts:
            # Link in between obrects
            # Skip the current object itself
            self.couplings[0].trees[Ts].set_couplings([i.trees[Ts] for i in self.couplings])
            for coupling in self.couplings[0].trees[Ts].couplings:
                coupling.fit_par = self.couplings[0].t1.fit_par
                coupling.fit_x0 = self.couplings[0].t1.fit_x0
                coupling.fit_sse = self.couplings[0].t1.fit_sse
            self.trees[Ts].get_bic(reset=True,fit=True,verbose=False)
            self.trees[Ts].get_energy(reset=True)
