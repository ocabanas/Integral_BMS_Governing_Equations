...
class Tree:
    ...
    def __init__(
        ...
        if x is None:
            ...
            self.dx = {"d0": pd.DataFrame()}  # {'d0' : pd.Series(dtype=float)}
            self.t = {"d0": pd.Series()}
        ...
        if initial_guess == None:
            self.x0_values = {key: deepcopy(np.asarray(np.atleast_1d(this_x.iloc[0, 0]), dtype=float)) for key, this_x in self.x.items()}
            #print("Setting initial guess as first point", self.x0_values)
        else:
            self.x0_values = initial_guess
        #print("Setting initial guess as first point", self.x0_values)
    ...
    def get_sse(self, fit=True, verbose=False):
        ...
        if self.couplings == None:
            ...
            model_key=(str(model) for model in self.couplings)
        if model_key in list(self.fit_sse.keys()):
            self.sse = deepcopy(self.fit_sse[model_key])
            return self.sse
        """
        #fit = True
        ...
        # === Case 2: coupled equations ===
            ...
            # Build ODE function
            def ODE(*args):
                return [f(*args) for f in lambdas]
        
        def SSE_deriv(params, ODE_func, X, dX_dt, verbose=False):
            n_samples, n_eq = X.shape
            try:
                # Compute model-predicted derivatives for all points
                dX_model = np.array([ODE_func(*X[i], *params) for i in range(n_samples)])
            except Exception as e:
                if verbose:
                    print("ODE evaluation failed:", e)
                    print(X[0], params)
                    print(ODE_func(X[0], params))
                return np.inf
        
            # Compute squared errors
            residuals = dX_dt - dX_model
            sse = np.sum(residuals ** 2)
        
            if verbose:
                print(f"SSE({params}) = {sse:.6f}")
            return sse
        def fit_params_from_derivatives(ODE_func, X, dX_dt, param_guess, bounds=None, verbose=False):
            """
            Fit parameters by minimizing SSE_deriv.
        
            Parameters
            ----------
            ODE_func : callable
                Function that returns ODE derivatives.
            X : ndarray
                State data (n_samples, n_eq)
            dX_dt : ndarray
                Numerical derivatives (n_samples, n_eq)
            param_guess : array-like
                Initial guess for parameters.
            bounds : tuple or None
                Bounds for optimizer (lower, upper).
            verbose : bool
            ...
            result = minimize(
                SSE_deriv,
                x0=np.asarray(param_guess, dtype=float),
                args=(ODE_func, X, dX_dt, verbose),
                method='L-BFGS-B',
                bounds=bounds
            )
            if verbose:
        ...
        def make_sse_function_from_ode(ode_func, t_eval, x0_fixed=None, param_fixed=None,
                                   optimize_x0=True, optimize_params=True, observed=None, verbose=False):
            """
            Build an SSE function for use with scipy.optimize.minimize using a lambdified ODE function.
            ...
            def sse_fun(x):
                # Map optimizer array to x0 and parameters
                idx = 0
                if optimize_x0:
                    x0 = x[idx:idx + x0_len]
                    idx += x0_len
                ...
                # Build ODE function in solve_ivp form
                """
                def f(t, y):
                    if not np.all(np.isfinite(y)) or np.any(np.abs(y) > 1e10):
                    ...
                    if len(params) > 0:
                        args.extend(params)
                    return np.array(ode_func(*args), dtype=float)
                """
                #@njit
                def f_numba(t, y, params):
                    # Write out ODE equations directly or wrap from ode_func
                    return np.asarray(ode_func(*y, *params), dtype=float)
        
                # Integrate ODE
                try:
                    """
                    sol_ivp = solve_ivp(f, (t_eval[0], t_eval[-1]), x0, t_eval=t_eval,
                                        method='RK45',
                                         rtol=1e-6, atol=1e-9)
                    if not sol_ivp.success:
                        raise ValueError("Integration failed")
                    
                    sol = sol_ivp.y.T  # shape (len(t_eval), n_vars)
                    """
                    #@njit
                    def integrate_rk4(f, t_eval, y0):
                        y = np.zeros((len(t_eval), len(y0)))
                        y[0] = y0
                        dt = np.diff(t_eval)
                        for i in range(len(dt)):
                            k1 = f(t_eval[i], y[i])
                            k2 = f(t_eval[i] + dt[i]/2, y[i] + dt[i]*k1/2)
                            k3 = f(t_eval[i] + dt[i]/2, y[i] + dt[i]*k2/2)
                            k4 = f(t_eval[i] + dt[i], y[i] + dt[i]*k3)
                            y[i+1] = y[i] + (dt[i]/6)*(k1 + 2*k2 + 2*k3 + k4)
                        return y
                    sol = integrate_rk4(lambda t, y: f_numba(t, y, params), t_eval, x0)
                except Exception as e:
                    if verbose:
                        print("Integration error:", e)
                    return np.inf
        
                # Compute SSE if observed is provided
                try:
                    sse = np.sum((sol - observed) ** 2)
                    """
                    plt.plot(t_eval,observed)
                    plt.plot(t_eval,sol)
                    plt.show()
                    """
                except Exception as e:
        ...
        if fit:
            ...
            if fit_key in self.fit_par and fit_key in self.fit_x0:
                ...
                self.par_values=deepcopy(self.fit_par[fit_key])
                self.x0_values=deepcopy(self.fit_x0[fit_key])
                ...
                for ds in self.x:
                    ...
                    xmat = this_x.to_numpy(dtype=float)
                    this_t = self.t[ds].to_numpy(dtype=float)
                    # Get x0 gess from data
                    cols = this_x.columns[:n_eq]  # take first n_equations columns
                    x0 = this_x.loc[0, cols].to_numpy(dtype=float)
                    # Integral minimization to optimize x0
                    # Generate SSE to minimize only x0
                    SSE = make_sse_function_from_ode(ODE, t_eval = this_t, x0_fixed=x0, param_fixed=None,
                                                    optimize_x0=True, optimize_params=False, observed=xmat, verbose=verbose)
                    try:
                        res = minimize(SSE, x0=x0, method='L-BFGS-B')
                    except Exception as e:
                        if verbose:
                ...
                for ds in self.x:
                    ...
                    this_x = self.x[ds].iloc[:, :n_eq]
                    xmat = this_x.to_numpy(dtype=float)
                    this_dx = self.dx[ds].iloc[:, :n_eq]
                    dxmat = this_dx.to_numpy(dtype=float)
                    this_t = self.t[ds].to_numpy(dtype=float)
                    cols = this_x.columns[:n_eq]  # take first n_equations columns
                    x0 = this_x.loc[0, cols].to_numpy(dtype=float)
                    ...
                    # struct: (x0,params)
                    guesses = [np.asarray(list(x0) + list(df_fit_par.x), dtype=float),
                          np.asarray( list(x0) + list(p0), dtype=float)]#,
                         #np.array( list(self.x0_values[ds]) + list(p0))]  # e.g. first row of data
                    if verbose:
                        print('List of guess:',guesses)
                    # Build SSE integral function
                    SSE = make_sse_function_from_ode(ODE, t_eval = this_t, x0_fixed=x0, param_fixed=p0,
                    ...
                    for i, guess in enumerate(guesses):
                        ...
                        try:
                            res = minimize(SSE, x0=guess, method='L-BFGS-B')
                    ...
                    if best_res is not None or hasattr(best_res, "x"):
                        ...
                        # If optimization is a success save parameters and goto final SSE
                        x_opt = np.asarray(best_res.x, dtype=float)
                        if np.all(np.isfinite(x_opt)):
                        ...
                        guesses = [np.asarray(df_fit_par.x, dtype=float),
                              np.asarray(p0, dtype=float)]  # e.g. first row of data
                        
                        # Build SSE integral function
                        SSE = make_sse_function_from_ode(ODE, t_eval = self.t[ds], x0_fixed=x0, param_fixed=p0,
        ...
        for ds in self.x:
            try:
                ...
                SSE = make_sse_function_from_ode(ODE, t_eval = this_t, x0_fixed=self.x0_values[ds], param_fixed=pars,
        ...
        # Done
        #self.fit_sse[model_key] = deepcopy(self.sse)
        return self.sse
    ...
    def dE_et(self, target, new, verbose=False):
        ...
        old_x0_values = deepcopy(self.x0_values)
        #
        ...
        if rep_res == -1:
            ...
            self.x0_values = old_x0_values
            return (
                np.inf,
                np.inf,
                np.inf,
                deepcopy(self.par_values),
                deepcopy(self.x0_values),
                nif,
                nfi,
            )
        ...
        if not list(self.x.values())[0].empty:
            ...
            x0_valuesOld = deepcopy(self.x0_values)
            #
            old = [target.value, [o.value for o in target.offspring]]
            # replace
            ...
            x0_valuesNew = deepcopy(self.x0_values)
            # leave the whole thing as it was before the back & fore
            self.et_replace(added, old, update_gof=False, verbose=verbose)
            self.bic = bicOld
            self.sse = deepcopy(sseOld)
            self.par_values = par_valuesOld
            self.x0_values = x0_valuesOld
            #
    ...
    def dE_rr(self, rr=None, verbose=False):
        ...
        # Root replacement
            ...
            if not list(self.x.values())[0].empty:
                ...
                newroot = self.replace_root(rr=rr, update_gof=False, verbose=verbose)
                if newroot == None:
                    return np.inf, np.inf, np.inf, self.par_values, self.x0_values
                bicNew = self.get_bic(reset=True, fit=True, verbose=verbose)
                par_valuesNew = deepcopy(self.par_values)
                x0_valuesNew = deepcopy(self.x0_values)
                # leave the whole thing as it was before the back & fore
    ...
    def mcmc_step(self, verbose=False, p_rr=0.05, p_long=0.45):
        ...
        # Long-range move
            ...
            if dice < paccept:
                ...
                self.x0_values = deepcopy(x0_valuesNew)
                self.get_bic(reset=True, fit=False, verbose=verbose)
                self.get_energy(reset=True)
                if verbose:
