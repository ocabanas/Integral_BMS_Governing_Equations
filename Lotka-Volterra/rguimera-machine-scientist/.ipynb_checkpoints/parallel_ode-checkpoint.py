...
class Parallel():
    ...
    def __init__(self, Ts, ops=OPS, variables=['x'], parameters=['a'],
                 max_size=50,
                 prior_par={}, x=None, y=None,initial_guess=None):
        # All trees are initialized to the same tree but with different BT
        Ts.sort()
        self.Ts = [str(T) for T in Ts]
        self.trees = {self.Ts[0]: Tree(ops=ops,
                                 variables=deepcopy(variables),
                                 parameters=deepcopy(parameters),
                                 prior_par=deepcopy(prior_par), x=x, y=y,initial_guess=initial_guess,
                                 max_size=max_size,
                                 BT=1)}
    ...
    def tree_swap(self):
        ...
        """
        # Bic from coupling swap:
        t1.fy,t2.fy = t2.fy,t1.fy
        EB1_=t1.get_bic(reset=False,fit=True)
        EB2_=t2.get_bic(reset=False,fit=True)
        t1.fy,t2.fy = t2.fy,t1.fy # As before
        """
        # The energy change
        DeltaE = np.float64(EB1) * (1./BT2 - 1./BT1) + \
                 np.float64(EB2) * (1./BT1 - 1./BT2)
        ...
        if random() < paccept:
            ...
            self.t1 = self.trees[self.Ts[0]]
            """
            self.trees[self.Ts[nT1]].get_bic(reset=True,fit=True)
            self.trees[self.Ts[nT1]].get_energy(bic=False,reset=True)
            self.trees[self.Ts[nT1]].fy.get_energy(bic=False,reset=True)
            
            self.trees[self.Ts[nT2]].get_bic(reset=True,fit=True)
            self.trees[self.Ts[nT2]].get_energy(bic=False,reset=True)
            self.trees[self.Ts[nT2]].fy.get_energy(bic=False,reset=True)
            """
            #print('Delta PT', DeltaE,paccept)
            #print(t1,t1.E)
            #print(t2,t2.E)
            #exit(1)
            #time.sleep(3)
