...
class Tree():
    ...
    def __init__(self, ops=OPS, variables=['x'], parameters=['a'],
                 prior_par={}, x=None, y=None,initial_guess=None, BT=1., PT=1.,
                 max_size=50,
    ...
    def build_from_string(self, string, verbose=False):
        ...
        self.__init__(ops=self.ops, prior_par=self.prior_par,
                      ...
                      parameters=parameters, variables=variables,initial_guess=self.x0_guess)
...
Node and new is a tuple [node_value, [list, of, offspring, values]]
    ...
    def get_sse(self, fit=True, verbose=False):
        ...
        parameters_x = [atomd_x[p] for p in self.parameters if p in list(atomd_x.keys())]
        
        try:
            flam_x = lambdify(
                variables_x + parameters_x, ex_x, [
                    "numpy",
        ...
        try:
            ...
            variables_y = [atomd_y[v] for v in self.fy.variables if v in list(atomd_y.keys())]
            parameters_y = [atomd_y[p] for p in self.fy.parameters if p in list(atomd_y.keys())]
            flam_y = lambdify(
                variables_y + parameters_y, ex_y, [
                    "numpy",
                    {'fac' : scipy.special.factorial}
                ])
        except Exception as e:
            ...
            if recover_fit_pars(str(self),str(self.fy)):
                    ...
                    def residuals(x,*a):
                        ...
                        """def ode(y):
                            ...
                            for v_x in variables_x:
                                if v_x.name == 'x': args_x.append(y[0])
                                elif v_x.name == 'y': args_x.append(y[1])
                            # Y Lambda input
                            args_y=[]
                            for v_y in variables_y:
                                if v_y.name == 'x': args_y.append(y[1])
                                elif v_y.name == 'y': args_y.append(y[0])
                            if len(pars_y)!=0: args_y+=pars_y
                            return [flam_x(*args_x),flam_y(*args_y)]
                        """
                        def ode(y):
                            #k1
                            args_x=[]
                            ...
                            args_x=[]
                            for v_x in variables_x:
                                if v_x.name == 'x': args_x.append(y[0]+k1[0]/2.)
                                elif v_x.name == 'y': args_x.append(y[1]+k1[1]/2.)
                            # Y Lambda input
                            args_y=[]
                            for v_y in variables_y:
                                if v_y.name == 'x': args_y.append(y[1]+k1[1]/2.)
                                elif v_y.name == 'y': args_y.append(y[0]+k1[0]/2.)
                            if len(pars_y)!=0: args_y+=pars_y
                            k2 = [h*flam_x(*args_x),h*flam_y(*args_y)]
                            #k3
                            args_x=[]
                            for v_x in variables_x:
                                if v_x.name == 'x': args_x.append(y[0]+k2[0]/2.)
                                elif v_x.name == 'y': args_x.append(y[1]+k2[1]/2.)
                            # Y Lambda input
                            args_y=[]
                            for v_y in variables_y:
                                if v_y.name == 'x': args_y.append(y[1]+k2[1]/2.)
                                elif v_y.name == 'y': args_y.append(y[0]+k2[0]/2.)
                            if len(pars_y)!=0: args_y+=pars_y
                            k3 = [h*flam_x(*args_x),h*flam_y(*args_y)]
                            #k4
                            args_x=[]
                            for v_x in variables_x:
                                if v_x.name == 'x': args_x.append(y[0]+k3[0])
                                elif v_x.name == 'y': args_x.append(y[1]+k3[1])
                            # Y Lambda input
                            args_y=[]
                            for v_y in variables_y:
                                if v_y.name == 'x': args_y.append(y[1]+k3[1])
                                elif v_y.name == 'y': args_y.append(y[0]+k3[0])
                            if len(pars_y)!=0: args_y+=pars_y
                        ...
                        return np.concatenate((np.subtract(sol[0],this_x.x.to_numpy()),
                                              np.subtract(sol[1],this_x.y.to_numpy())))
                        #h=(time[1]-time[0])
                        #sol=scipy.integrate.solve_ivp(ode, t_span=[time[0],time[-1]], y0=[x0,y0], method='RK45',t_eval=time,first_step=h,max_step=h,rtol=1e-1 )
                        #return np.concatenate((np.subtract(sol.y[0],this_x.x.to_numpy()),
                        #                      np.subtract(sol.y[1],this_x.y.to_numpy())))
                    try:
                        ...
                        if len(parameters_y)!=0:
                            try:
                                ...
                                res_fit_y = curve_fit(
                                    feval, xmat, this_y,
                                    p0=[self.fy.par_values[ds][p.name]
                                        for p in parameters_y],
                                    maxfev=1000000,
                                )
                                x0_fit+=[val for val in res_fit_y[0]]
                                x0_pars+=[self.fy.par_values[ds][p.name] for p in parameters_y]
                            except Exception as e:
                                if verbose: print(e)
                                res_fit_y=None
                                x0_fit+=[1. for p in parameters_y]
                                x0_pars+=[self.fy.par_values[ds][p.name] for p in parameters_y]
                        else:
                            res_fit_y=None
                        
                        if False in np.isfinite(x0_fit):
                            warnings.warn("Non finite value encountered in x0: setting to 1.")
                        if any(np.abs(x)>1e10 for x in x0_fit):
                            warnings.warn("Overflow value encountered in x0: setting to 1.")
                        #Optimization
                        b=[(-1e15,)*len(x0_fit),(1e15,)*len(x0_fit)]
                        this_x = self.x[ds]
                        res_fit=scipy.optimize.least_squares(residuals, x0=x0_fit,method='trf',bounds=b)#,max_nfev=500000)
                        res_pars=scipy.optimize.least_squares(residuals, x0=x0_pars,method='trf',bounds=b)#,max_nfev=500000)
                        res=min(res_fit,res_pars,key=lambda x:x['cost'])
                        if False in np.isfinite(res.x):
                            raise ValueError
                        if any(np.abs(x)>1e10 for x in res.x):
                            warnings.warn(f"Overflow value encountered in optimized parameters: {res.x}")
                        x0,y0,*pars_y=res.x
                        # Save fit pars
                        ...
                        for p in self.parameters:
                            ...
                            #self.fy.par_values[ds][p] = 1.
                        self.fit_par[str(self)][str(self.fy)][ds]=deepcopy(self.par_values[ds])
                        self.fy.fit_par[str(self.fy)][str(self)][ds]=deepcopy(self.fy.par_values[ds])
                ...
                for ds in self.x:
                    ...
                    def residuals(x,*a):
                        #print('sse min 1',x)
                        x0,y0,*pars=x
                        pars_x,pars_y=pars[0:len(parameters_x)],pars[len(parameters_x):]
                        time=this_x.t.to_numpy()
                        #residuals
                        """
                        def ode(y):
                    ...
                    try:
                        ...
                        try:
                            ...
                            res_fit_x = curve_fit(
                                ...
                                p0=[self.par_values[ds][p.name]
                                    for p in parameters_x],
                                maxfev=1000000,
                            )
                            x0_fit+=[val for val in res_fit_x[0]]
                            x0_pars+=[self.par_values[ds][p.name] for p in parameters_x]
                        except Exception as e:
                            if verbose: print(e)
                            res_fit_x=None
                            x0_fit+=[1. for p in parameters_x]
                            x0_pars+=[self.par_values[ds][p.name] for p in parameters_x]
                        
                        if len(parameters_y)!=0:
                            try:
                                this_x, this_y = self.fy.x[ds], self.fy.x[ds]['dx'].to_numpy()
                        ...
                        """
                        if verbose:
                            print('Fit pars x',res_fit_x[0])
                            print('Fit pars y',res_fit_y[0])
                        """
                        if False in np.isfinite(x0_fit):
                            warnings.warn("Non finite value encountered in x0: setting to 1.")
                            x0_fit=[value if np.isfinite(value) else 1. for value in x0_fit]
                        if any(np.abs(x)>1e10 for x in x0_fit):
                            warnings.warn("Overflow value encountered in x0: setting to 1.")
                            x0_fit=[value if np.abs(value)<1e10 else 1. for value in x0_fit]
                        #Optimize pars
                        b=[(-1e15,)*len(x0_fit),(1e15,)*len(x0_fit)]
                        this_x = self.x[ds]
                        if verbose:
                        ...
                        try:
                            res_fit=scipy.optimize.least_squares(residuals, x0=x0_fit,method='trf', bounds=b)#,max_nfev=1000000,verbose=2)
                        except:
                            res_fit={'cost':np.inf}
                        try:
                            res_pars=scipy.optimize.least_squares(residuals, x0=x0_pars,method='trf', bounds=b)#,max_nfev=1000000,verbose=2)
                        except:
                        ...
                        if verbose:
                            ...
                            print(f'#Cannot_fit:{ex_x}|{ex_y}')
                            print('Imput:','x0',x0_fit)
                            print(f'Canonicals:{self.canonical()}|{self.fy.canonical()}')
                            print(res_fit_x)
                            print(res_fit_y)
                            print('Lambd X',ex_x,atomd_x,variables_x,parameters_x)
                            print('Lambd Y',ex_y,atomd_y,variables_y,parameters_y)
                        ...
                        self.fit_par[str(self)][str(self.fy)][ds]=deepcopy(self.par_values[ds])
                        self.fy.fit_par[str(self.fy)][str(self)][ds]=deepcopy(self.fy.par_values[ds])
                        #print('Final pars_x', ex_x,self.par_values[ds])
                        #print('Final pars_y', ex_y,self.fy.par_values[ds])
        ...
        for ds in self.x:
            this_x = self.x[ds]
            #print('parsx',[self.par_values[ds][p.name] for p in parameters_x])
            #print('parsy',[self.fy.par_values[ds][p.name] for p in parameters_y])
            #RK
            def sse(x0,y0,pars_x,pars_y):
                ...
                #h=(time[1]-time[0])
                #sol=scipy.integrate.solve_ivp(ode, t_span=[time[0],time[-1]], y0=[x0,y0], method='RK45',t_eval=time,first_step=h,max_step=h,rtol=1e-1 )
                #return np.sum(np.square(np.subtract(sol.y[0],this_x.x.to_numpy())))
            try:
                if str(self) not in list(self.x0.keys()):
                ...
                #print(self.x0)
                #print(self.fy.x0)
                x0,y0=self.x0[str(self)][str(self.fy)][ds],self.fy.x0[str(self.fy)][str(self)][ds]
                ...
                if np.isnan(se) > 0:
                    if verbose: print(ex_x,ex_y,pars_x,pars_y)
                    raise ValueError
    ...
    def get_bic(self, reset=True, fit=True, verbose=False):
        ...
        if type(self.fy) != type(None):
            ...
            # Calculate the BIC
            parameters = set([p.value for p in self.fy.ets[0]
                              if p.value in self.fy.parameters])
            k = 1 + len(parameters)
            for ds in self.x:
                n = len(self.x[ds])
                BIC += (k - n) * np.log(n) + n * (np.log(2. * np.pi) + log(sse_y[ds]) + 1)
        
        if reset == True:
            self.bic = BIC
            if type(self.fy) != type(None):
                self.fy.bic=self.fy.get_bic(reset=False)
                """if self.bic != self.fy.bic:
                    print('Wrong bic reset:',self.bic,self.fy.bic)
                    print()
                """
        if verbose: print('BIC',BIC,sse,parameters)
        return BIC
    ...
    def get_energy(self, bic=False, reset=False, verbose=False):
        ...
        if type(self.fy) != type(None):
            if bic == True:
                EBY = self.fy.get_bic(reset=reset, verbose=verbose) / 2.
            else:
                EBY = self.fy.bic / 2.
        """
        ...
        if reset:
            ...
            self.E = EB + EP
        # Done
        if verbose: print('GEner',EB + EP, EB, EP)
        """
        if type(self.fy) != type(None):
            return EB + EBY + EP, EB, EP
        else:
        """
        return EB + EP, EB, EP
...
the representatitve and return -2.
        ...
        try:             # We've seen this canonical before!
            raise KeyError
            rep, rep_energy, rep_par_values = self.representative[canonical][canonical_fy]
            ...
            self.representative[canonical][canonical_fy] = (str(self), new_energy,
...
a tuple [node_value, [list, of, offspring, values]].
        ...
        #
        old_bic_y, old_sse_y, old_energy_y = self.fy.bic, deepcopy(self.fy.sse), self.fy.E
        ...
        if rep_res == -1:
            ...
            #
            self.fy.bic, self.fy.sse, self.fy.E = old_bic_y, deepcopy(old_sse_y), old_energy_y
            self.fy.par_values = old_par_values_y
            #
            return np.inf, np.inf, np.inf, deepcopy(self.par_values),deepcopy(self.fy.par_values), nif, nfi
    ...
    def dE_rr(self, rr=None, verbose=False):
        ...
        if rr == None:
            if not self.is_root_prunable():
                return np.inf, np.inf, np.inf, self.par_values,deepcopy(self.fy.par_values)
            ...
            if rep_res == -1:
                ...
                self.fy.par_values = old_par_values_y
                #
                return np.inf, np.inf, np.inf, deepcopy(self.par_values),deepcopy(self.fy.par_values)
            # leave the whole thing as it was before the back & fore
            self.replace_root(rr=oldrr, update_gof=False, verbose=verbose)
            self.bic, self.sse, self.E = old_bic, deepcopy(old_sse), old_energy
            self.par_values = old_par_values
    ...
    def mcmc_step(self, verbose=False, p_rr=0.05, p_long=.45):
        ...
        if topDice < p_rr:
            if random() < .5:
                ...
                dE, dEB, dEP, par_valuesNew,par_valuesNew_y = self.dE_rr(rr=None,
                                                         verbose=verbose)
                ...
                if dice < paccept:
                    ...
                    #if paccept<1: print('Increase energy2:',str(self),self.E)
                    """print()
                    print('1',dE,paccept,dice)
                    print()"""
                    if verbose:
                ...
                dE, dEB, dEP, par_valuesNew, par_valuesNew_y = self.dE_rr(rr=newrr,
                                                         verbose=verbose)
            ...
            while not nready:
                if len(target.offspring) == 0:
                    ...
                    if self.ops[new] == self.ops[target.value]:
                        nready = True
            dE, dEB, dEP, par_valuesNew, par_valuesNew_y = self.dE_lr(target, new,
                                                     verbose=verbose)
        ...
        # Elementary tree (short-range) move
            ...
            dE, dEB, dEP, par_valuesNew, par_valuesNew_y, nif, nfi = self.dE_et(target, new,
                                                               verbose=verbose)
            if verbose:
