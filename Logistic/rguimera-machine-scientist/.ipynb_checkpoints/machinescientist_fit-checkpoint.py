...
src='/export/home/oriolca/BMS_ODE/Logistic/rguimera-machine-scientist/'
sys.path.append(src)
from mcmc_fit import *
from parallel_fit import *
    ...
    while runs < resets:
        try: #Sometimes a NaN error appears. Therefore we forget the current MCMC and start again.
            ...
            for i in range(1,steps_prod+1):
                ...
                BT1,BT2=pms.tree_swap() # Attempt to swap two randomly selected consecutive temps
                ...
                if (i % 10) == 0:
                    ...
                    print(f'Progress: {int(float(i*100)/float(steps_prod))}%  | {round(1./(end-start),2)} MCs/s | Time left: {round(float(steps_prod-i)*float(end-mc_start)/(60.*(i)),2)}min. | {str(pms.t1)}', end='\r')
            runs+=1
        except Exception as e:
            #q
            print('Error during MCMC evolution:')
            ...
            print('Current energy',pms.t1.E)
            print('Restarting MCMC')
        del pms, last_seen_by_can, last_seen_by_str,BT1,BT2
        ...
        try: #Sometimes a NaN error appears. Therefore we forget the current MCMC and start again.
            ...
            while annealing_model.BT>=0.001:
                ...
                if annealing_model.E < lowest_ann:
                    #if pms.t1.E==float('NaN'): print('NaN in best model mdl')
                    lowest_ann, ann_model = copy(annealing_model.E), deepcopy(annealing_model)
