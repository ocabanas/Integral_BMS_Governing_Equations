...
Node and new is a tuple [node_value, [list, of, offspring, values]]
    ...
    def get_sse(self, fit=True, verbose=False):
            ...
            if len(parameters) == 0: # Nothing to fit, goto except and fit initial value
                ...
                for ds in self.x:
                    ...
                    try:
                        ...
                        #print('Params from diff',res_fit_deriv[0])
                        x0=[np.abs(this_y[0])]+[val for val in res_fit_deriv[0]]
                        #print(x0)
                    except Exception as e:
                        #print(e)
                        if verbose: print(e)
                        res_fit_x=None
                        x0=[np.abs(this_y[0])]+[self.par_values[ds][p.name]
                                    for p in parameters]
                    ...
                    try:
                        ...
                        try:
                            ...
                            #print('Params from ode fit 1',res[0])
                            if True in np.isnan(feval(0.,*res[0])):
                                raise
                            ...
                            #print('Params from ode fit 2',res_noffset[0])
        ...
        for ds in self.x:
            ...
            #print('Par values sse',self.par_values[ds])
            #RK
            def feval(x0, *params):
                ode=[x0]
                h=self.x[ds].t[1]-self.x[ds].t[0]
