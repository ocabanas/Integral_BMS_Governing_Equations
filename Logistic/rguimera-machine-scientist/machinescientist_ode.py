...
priors = {
    ...
    "v1_p8": "Prior/final_prior_param_sq.named_equations.nv1.np8.2017-10-18 18:07:35.261518.dat",
        ...
        try:  # Sometimes a NaN error appears. Therefore we forget the current MCMC and start again.
            ...
            for i in range(1, steps_prod + 1):
                ...
                if (i % 10) == 0:
                    ...
                    print(
                        f"Progress: {int(float(i*100)/float(steps_prod))}%  | {round(1./(end-start),2)} MCs/s | Time left: {round(float(steps_prod-i)*float(end-mc_start)/(60.*(i)),2)}min. | {str(pms.t1)}",
                        end="\r",
                    )
            runs += 1
