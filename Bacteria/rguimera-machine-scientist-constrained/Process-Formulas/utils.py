...
def go_untouchable(start, end, key, symbol_type="var", force_replace=False):
    ...
    if symbol_type == "var":
        ...
        if force_replace:
            return (
                "|" + "#" * (num_token_1 - 1) + str(key) + "#" * (num_token_2 - 1) + "|"
            )
        else:
            return " " * num_token_1 + str(key) + " " * num_token_2
    else:
...
def check_undefined_symbols(exp):
    symbols = (
        list_to_regexp_or(LATEX_NOARG_SYMBOLS.keys())
        + list_to_regexp_or(LATEX_ARG_SYMBOLS.keys())
        + list_to_regexp_or(OTHER_SYMBOLS)
    )
...
def check_derived_expressions(exp):
    m = re.search(
        "[ (]*(d ?([a-zA-Z]|\|@*[0-9]@*\||\^[a-zA-Z0-9])[ )]*/[( ]*d ?([a-zA-Z]|\|@*[0-9]@*\||\^[a-zA-Z0-9]))[ )]*",
        exp,
    )
    if m:
...
def prepare_function_candidates(exp, function_candidates):
    for m in re.finditer(
        "(([a-zA-Z]+)(_{?[\\\\a-zA-Z0-9]+}?)?(\^{?[\\\\()\-+*/a-zA-Z0-9]+}?)?)\(", exp
    ):
        if exp[m.start() - 1] == "\\":
            symbol = "\\" + m.group(2)
            if (
                symbol in LATEX_NOARG_SYMBOLS
                and LATEX_NOARG_SYMBOLS[symbol]["type"] == "var"
            ):
...
FUNCTION_TERMS = [
    ...
    ";",
    "|",
]  # For example, f(a+b) is not function, place + here and it will be treated as function.
SPLIT_OPS = [
    "=",
    "<",
    ">",
...
NO_SPLIT_WORDS_SYMBOLS = [
    "*",
    "\\times",
    "\\cdot",
]  # Don't split words when this symbols appear in the expression
SPECIAL_NUMBERS = [
    "Ar",
    "Be",
    "Bi",
...
LATEX_NOARG_SYMBOLS = {
    ...
    "\\over": {
        ...
        "type": "op",
        "replace": False,
    },  # Replace { a*b +c \over a+b } with ( a*b +c )/( a+b )
