...
class Tree():
    ...
    def __init__(self, ops=OPS, variables=['x'], parameters=['a'],
                 ...
                 max_size=50,
                 root_value=None, from_string=None,constraint=None):
    ...
    def build_from_string(self, string, verbose=False):
        ...
        self.__init__(ops=self.ops, prior_par=self.prior_par,
                      ...
                      parameters=parameters, variables=variables,constraint=self.constraint)
...
Node and new is a tuple [node_value, [list, of, offspring, values]]
    ...
    def get_sse(self, fit=True, verbose=False):
        ...
        parameters = [atomd[p] for p in ['_a_']+self.parameters if p in list(atomd.keys())]
        #print(ex,parameters)
        try:
            flam = lambdify(
            ...
            return self.sse
        if fit:
            if len(parameters) == 0: # Nothing to fit, goto except and fit initial value
                for ds in self.x:
                ...
                for ds in self.x:
                    ...
                    def feval(x,x0):
                        ...
                        for steps in range(len(this_y)-1):
                            ...
                            ode.append(ode[-1] + 1./6*(k1+2.*k2+2.*k3+k4))
                        return ode
                    try: #Fit initial value
                        ...
                        if True in np.isnan(feval(0.,*res[0])):
                            raise
            ...
            elif (str(self) in self.fit_par) and (str(self) in self.x0): # Recover previously fit parameters
                ...
                for ds in self.x:
                    ...
                    try:
                        ...
                        res_fit_deriv = curve_fit(
                            feval, d_xmat, deriv_y,
                            p0=[self.par_values[ds][p.name]
                                for p in parameters],
                            maxfev=1000000,
                        )
                        x0_fit=[np.abs(this_y[0])]+[val for val in res_fit_deriv[0]]
                        x0_par=[np.abs(this_y[0])]+[self.par_values[ds][p.name] for p in parameters]
                        #print(x0)
                    except Exception as e:
                        #print(e)
                        if verbose: print(e)
                        res_fit_x=None
                        x0_fit=[np.abs(this_y[0])]+[1. for p in parameters]
                        x0_par=[np.abs(this_y[0])]+[self.par_values[ds][p.name] for p in parameters]
                    ...
                    try:
                        ...
                        try:
                            res_fit = curve_fit(
                                ...
                                p0=x0_fit,
                                maxfev=50000,
                                bounds=(tuple([-np.inf]+[-np.inf for p in parameters]),
                            ...
                            try:
                                ...
                                res=min(res_fit,res_par,key=lambda x: np.sum(x[2]['fvec']))
                            except:
                                res=res_fit
                            #print(res_fit)
                            #print(res_par)
                            ...
                            if verbose:
                                ...
                                print(x0_fit)
                                print([this_y[0]]+[self.par_values[ds][p.name] for p in parameters])
                                print(traceback.format_exc())
                            ...
                            res_noffset = curve_fit(
                                feval_noffset, xmat, this_y,
                                p0=x0_fit[1:],
                                bounds=(tuple(-np.inf if p.name!='_a_' else 0. for p in parameters),
                                       tuple(np.inf for p in parameters)),
                                maxfev=50000,
                            )
                            #Format result with initial and par values for Except case
                            res=[]
                            form=[this_y[0]]+[res_noffset[0][i] for i in range(len(res_noffset[0]))]
                        ...
                        if verbose:
                            ...
                            print([self.par_values[ds][p.name] for p in parameters])
                            print(traceback.format_exc())
