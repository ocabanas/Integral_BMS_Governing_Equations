...
class Tree:
    ...
    def get_sse(self, fit=True, verbose=False):
        ...
        # print('_a_*B + '+str(self))
        if self.constraint == None:
            ex = sympify(str(self))
        else:
            ex = sympify(self.constraint[0] + str(self) + self.constraint[1])
        ...
        parameters = [
            atomd[p] for p in ["_a_"] + self.parameters if p in list(atomd.keys())
        ]
        # print(ex,parameters)
        try:
            flam = lambdify(
            ...
            if verbose:
                print("Get SSE, lambdify error", e)
            self.sse = dict([(ds, np.inf) for ds in self.x])
            return self.sse
        if fit:
            if (
                len(parameters) == 0
            ):  # Nothing to fit, goto except and fit initial value
                for ds in self.x:
                    for p in self.parameters:
                        self.par_values[ds][p] = 1.0
                if str(self) not in self.x0:
                    self.x0[str(self)] = {}
                for ds in self.x:
                    ...
                    def feval(x, x0):
                        ode = [x0]
                        h = self.x[ds].t[1] - self.x[ds].t[0]
                        for steps in range(len(this_y) - 1):
                            # k1
                            args = []
                            if len(variables) == 1:
                                args = [ode[-1]]
                            ...
                            ode.append(
                                ode[-1] + 1.0 / 6 * (k1 + 2.0 * k2 + 2.0 * k3 + k4)
                            )
                        return ode

                    try:  # Fit initial value
                        res = curve_fit(
                            feval, xmat, this_y, p0=[np.abs(this_y[0])], maxfev=50000
                        )
                        self.x0[str(self)][ds] = res[0][0]
                        if True in np.isnan(feval(0.0, *res[0])):
                            raise

                    except Exception as e0:
                        if verbose:
                            print(ds)
                            print("Get SSE, fit 1 error", e0)
                            print([this_y[0]])
                            print(traceback.format_exc())
                        self.x0[str(self)][ds] = this_y[0]

            elif (str(self) in self.fit_par) and (
                str(self) in self.x0
            ):  # Recover previously fit parameters
                self.par_values = deepcopy(self.fit_par[str(self)])
                ...
                for ds in self.x:
                    ...
                    try:
                        deriv_x, deriv_y = self.x[ds], self.x[ds]["dx"].to_numpy()
                        # print('this_y,x',this_y)
                        d_xmat = [deriv_x[v.name].to_numpy() for v in variables]

                        def feval(x, *params):
                            args = [xi for xi in x] + [p for p in params]
                            return flam(*args)
                        if ds in self.x0[str(self)]:
                            x0_fit = [self.x0[str(self)][ds]]
                            x0_par = [self.x0[str(self)][ds]]
                        else:
                            x0_fit = [np.abs(this_y[0])]
                            x0_par = [np.abs(this_y[0])]
                        # Fit the parameters
                        res_fit_deriv = curve_fit(
                            feval,
                            d_xmat,
                            deriv_y,
                        ...
                        x0_fit = x0_fit + [val for val in res_fit_deriv[0]]
                        x0_par = x0_par + [self.par_values[ds][p.name] for p in parameters]
                        # print(x0)
                    except Exception as e:
                        # print(e)
                        if verbose:
                        ...
                        if ds in self.x0[str(self)]:
                            ...
                            x0_fit = [np.abs(this_y[0])]
                            x0_par = [np.abs(this_y[0])]
                        x0_fit = x0_fit + [1.0 for p in parameters]
                        x0_par = x0_par + [
                    ...
                    def feval(x, *params):
                        ...
                        for steps in range(len(this_y)-1):
                            ...
                            if len(parameters)!=0:
                                args += [p for p in params]
                            ode.append(ode[-1] +h*flam(*args))
                        return ode
                    """
                    try:
                        # Fit the parameters and offset
                        try:
                            res_fit = curve_fit(
                                feval,
                                xmat,
                                ...
                                maxfev=100000,
                                bounds=(
                                    tuple([-np.inf] + [-np.inf for p in parameters]),
                                    tuple([np.inf] + [np.inf for p in parameters]),
                                ),
                                full_output=True,
	                            )
                            if verbose:
                                print('Fit from fit',np.sum(np.square(res_fit[2]["fvec"])),np.sum(res_fit[2]["fvec"]))
                            try:
                                res_par = curve_fit(
                                    feval,
                                    xmat,
                                    ...
                                    bounds=(
                                        tuple(
                                            [-np.inf] + [-np.inf for p in parameters]
                                        ),
                                        tuple([np.inf] + [np.inf for p in parameters]),
                                    ),
                                    full_output=True,
                                ...
                                if verbose:
                                    print('Fit from par',np.sum(np.square(res_par[2]["fvec"])),np.sum(res_par[2]["fvec"]))
                                res = min(
                                    res_fit, res_par, key=lambda x: np.sum(np.square(x[2]["fvec"]))
                                )
                            ...
                            if verbose:
                                print(ds)
                                print("Get SSE, fit 2 error(x0)", e1)
                                print(x0_fit)
                                print(
                                    [this_y[0]]
                                    + [self.par_values[ds][p.name] for p in parameters]
                            ...
                            # Fit not successful: Fitting without offset
                            def feval_noffset(x, *params):
                                ode = [this_y[0]]
                                h = self.x[ds].t[1] - self.x[ds].t[0]
                                for steps in range(len(this_y) - 1):
                                    # k1
                                    ...
                                    ode.append(
                                        ode[-1]
                                        + 1.0 / 6 * (k1 + 2.0 * k2 + 2.0 * k3 + k4)
                                    )
                                return ode

                            res_noffset = curve_fit(
                                feval_noffset,
                                xmat,
                                this_y,
                                p0=x0_fit[1:],
                                ...
                                #    tuple([np.inf] + [np.inf for p in parameters])
                                #    ,
                                #    tuple(np.inf for p in parameters),
                                #),
                                maxfev=50000,
                            )
                            # Format result with initial and par values for Except case
                            res = []
                            form = [this_y[0]] + [
                                res_noffset[0][i] for i in range(len(res_noffset[0]))
                            ]
                            res = tuple([form, [form, form]])
                            if verbose:
                                print('No offset fit successful')
                                print('results',res)
                                print('curvefit',res_noffset)
                                print('curvefit deriv',res_fit_deriv)
                            if np.isinf(res_noffset[1]).any():
                                if verbose:
                                    print('Inf in cov array. Raising error')
                                raise

                        self.x0[str(self)][ds] = res[0][0]
                        # print('curve fit end')self.x0[str(self)][ds]=res[0][0]
                        # Reassign the values of the parameters
                        self.par_values[ds] = dict(
                            [
                                (parameters[i].name, res[0][i + 1])
                        ...
                        if verbose:
                            print(ds)
                            print("Get SSE, fit 2 error(no x0)", e2)
                            print('Par values',[self.par_values[ds][p.name] for p in parameters])
                            print('Par values (fit)',res_fit_deriv)
                            print(traceback.format_exc())
                        ...
                        for p in self.parameters:
                            if p not in self.par_values[ds]:
                                self.par_values[ds][p] = 1.0
                        for i,val in enumerate(res_fit_deriv[0]):
                            self.par_values[ds][parameters[i].name]=val
                        if verbose:
                            print('Setting deriv fit params',self.par_values[ds])
                        self.x0[str(self)][ds] = this_y[0]
                        if verbose:
                            print(
        ...
        for ds in self.x:
            ...
            """ar = [np.array(xi) for xi in xmat] + \
                 [self.par_values[ds][p.name] for p in parameters]"""
            ar = [self.par_values[ds][p.name] for p in parameters]

            # print('Initial SSE paprams',ds,parameters,ar)
            # print('Par values',self.par_values[ds])
            # RK
            def feval(x0, *params):
                ode = [x0]
                h = self.x[ds].t[1] - self.x[ds].t[0]
                for steps in range(len(this_y) - 1):
                    ...
                    ode.append(ode[-1] + 1.0 / 6 * (k1 + 2 * k2 + 2 * k3 + k4))
                return ode
            ...
            try:
                if str(self) not in list(self.x0.keys()):
                    ...
                    exit(1)
                init = self.x0[str(self)][ds]
                integr = feval(init, *ar)
                se = np.square(this_y - feval(init, *ar))
