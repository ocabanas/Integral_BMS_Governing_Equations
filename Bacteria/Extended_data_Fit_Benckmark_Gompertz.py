...
string = '(_a0_ + (_a1_ * (B * (log((_a2_ / B))))))' #Gompertz

def loss_ode(param):
    model=[]
    for value in x[col].B.to_numpy():
        model.append(value*param[0]*np.log(param[1]/value)+param[2])
    return np.sum(np.square(np.subtract(model,x[col].dx.to_numpy())))
def ode0(y,t,a,b,c):
    return y*a*np.log(b/y)+c
...
for col in list(x.keys()):
    # Uncomment to fit logistic model
    """
    ...
    if np.isinf(bms_fulldata_new.sse[col]) or bms_fulldata_new.par_values[col]['_a1_']<0.:
        ...
        fit = scipy.optimize.minimize(loss_ode,[1.,1.,1.],bounds=((0.,10.),(0.,10.),(None,None))).x
        """
...
f_name=f'{folder_name}/gompertz_test_fit_t.pkl'
file = open(f_name,'wb')
