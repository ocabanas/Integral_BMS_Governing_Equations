...
with open(f'./models/{file_name[:-4]}_BMS_SMOOTHED.pkl', 'wb') as f:
    # A new file will be created
    pickle.dump({'x':best_model_x,'y':best_model_y}, f)
plt.plot(dlsx)
plt.plot(dlsy)
plt.savefig(f'./models/{file_name[:-4]}_BMS_SMOOTHED_dl.pdf',format='pdf')
plt.close()

plt.plot (x_hat, dxdt_hat, label='dx data')
plt.plot (x_hat, best_model_x.predict(best_model_x.x)['d0'], label='dx model')
plt.plot (y_hat, dydt_hat, label='dy data')
plt.plot (y_hat, best_model_y.predict(best_model_x.x)['d0'], label='dy model')
plt.savefig(f'./models/{file_name[:-4]}_BMS_SMOOTHED_predictions.pdf',format='pdf')
plt.close()
del data,x,y,h,best_model_x,best_model_y
if smooth:
    del x_hat,dxdt_hat
else:
    del B
